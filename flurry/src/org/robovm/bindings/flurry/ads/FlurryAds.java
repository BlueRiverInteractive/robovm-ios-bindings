...
public class FlurryAds extends NSObject {
     ...
     * 
     * The space simply represents the placement of the ad in your app and should be unique for each placement. For example, if
     * you are displaying a full screen ad on your splash screen and after level completion, you may have the following spaces
     ...
     * @see #displayAd(String, UIView, UIViewController)
    ...
    /** Display an ad for the given {@code space}.
     * 
     * This method will display an ad if one is ready for display on the device.
     *
     * The {@code space} simply represents the placement of the ad in your app and should be unique for each placement. Only one ad
     * will show at a time for any given ad space. For example, if you are displaying a full screen ad on your splash screen and
     * after level completeion, you may have the following spaces {@code "SPLASH_AD"} and {@code "LEVEL_AD"}.
     *
     *
     ...
     *            #fetchAdForSpace:frame:size. Note view is not used for interstitials.
     * @param viewControllerForPresentation The viewController to show the fullscreen ad modally. be splash screen for SPLASH_AD.
     * 
     * @since 5.4.0
     * @see #fetchAd(String, CGRect, FlurryAdSize) for details on retrieving an ad.
     * @see #isAdReady(String) for details on verifying is an ad is ready to be displayed.
     * @see #removeAd(String) for details on manually removing an ad from a view.
     * @see FlurryAdDelegate#shouldDisplay(String, boolean) for details on controlling whether an ad will display immediately
     *      before it is set to be rendered to the user.
     * @see FlurryAdDelegate#didFailToRender(String, org.robovm.apple.foundation.NSError) for details on notification of error in
     *      rendering an ad for this request. */
    ...
    /** Fetch and Display an ad for the given {@code space}.
     * 
     *
     * This method will display an ad if one is available from the Flurry server for this user.
     *
     * If this method returns {@code true}, an ad is available for the space within {@code timeout}.<br/>
     * This is a blocking method that allows you to change the user experience based on availability of an ad. If you would like
     * to display an ad asynchronously, just set timeout to 0. This is useful in the case of banners for instance where the user
     ...
     * rely exclusively on the relevant delegate methods listed below.<br/>
     * The {@code space} simply represents the placement of the ad in your app and should be unique for each placement. Only one ad
     * will show at a time for any given ad space. For example, if you are displaying a full screen ad on your splash screen and
     * after level completeion, you may have the following spaces {@code "SPLASH_AD"} and {@code "LEVEL_AD"}.
     *
     ...
     *            space.
     * @param size The default size of an ad space. This can be overriden on the server. See {@link FlurryAdSize} for allowable
     *            values.
     * 
     * @since 5.4.0
     * @see #isAdReady(String) for details on verifying is an ad is ready to be displayed.
     * @see #removeAd(String) for details on manually removing an ad from a view.
     * @see FlurryAdDelegate#didReceiveAd(String) for details on the notification of ads being received.
     * @see FlurryAdDelegate#didFailToReceiveAd(String, org.robovm.apple.foundation.NSError) for details on notification of
     *      failure to receive ads from this request.
     * @see FlurryAdDelegate#shouldDisplay(String, boolean) for details on controlling whether an ad will display immediately
     *      before it is set to be rendered to the user.
     * @see FlurryAdDelegate#didFailToRender(String, org.robovm.apple.foundation.NSError) for details on notification of error in
     *      rendering an ad for this request. */
    @Method(selector = "fetchAndDisplayAdForSpace:view:viewController:size:")
    public static native void fetchAndDisplayAd (String space, UIView viewContainer,
        UIViewController viewControllerForPresentation, FlurryAdSize size);
     ...
     * 
     * This method allows you to request test ads from the server. These ads do not generate revenue so it is CRITICAL this call
     * is removed prior to app submission.
     * 
     * @since 4.0
     ...
     * 
     * UserCookies allow the developer to specify information on a user executing an ad action. There is one UserCookie object,
     * and on each ad click that UserCookie is transmitted to the Flurry servers. The UserCookie key/value pairs will be
     * transmitted back to the developer via the app callback if one is set. This is useful for rewarded inventory, to identify
     * which of your users should be rewarded when a reward callback is sent.
     * 
     ...
     * 
     * Keywords allow the developer to specify information on a user executing an ad action for the purposes of targeting. There
     * is one keywords object that is transmitted to the Flurry servers on each ad request. If corresponding keywords are matched
     * on the ad server, a subset of targeted ads will be delivered. This allows partners to supply information they track
     * internally, which is not available to Flurry's targeting system.
     * 
     * Calling this method with a nil or empty dictionary has no effect. Calling this method a second time with a valid dictionary
    ...
    public static native void addCustomAdNetwork (Class<? extends FlurryCustomAdNetwork> adNetworkClass,
        FlurryCustomAdNetworkProperties adNetworkProperties);
}
