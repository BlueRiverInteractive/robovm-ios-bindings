...
public class Xplode extends NSObject {
    ...
    // / This method should be called on every app launch from within applicationDidFinishLaunching:withOptions: in your app
// delegate.
    // /
    ...
    // / Initializes the Xplode SDK using the assigned app credentials and allows all subsequent SDK calls to be executed. Use
// this method if you want to use Xplode push notifications.
    // / This method should be called on every app launch from within applicationDidFinishLaunching:withOptions: in your app
// delegate.
    // /
    // / @note The classes IDDCore and IddictionSDK have been deprecated, please use Xplode as an entry point.
    ...
    public static native void initialize (String appHandle, String appSecret, UIApplicationLaunchOptions launchOptions,
        @Block InitializeHandler initializeHandler);

    // / Initializes the Xplode SDK using the assigned organization credentials and allows all subsequent SDK calls to be
// executed. Use this method if you want to use Xplode push notifications.
    // / This method should be called on every app launch from within applicationDidFinishLaunching:withOptions: in your app
// delegate.
    // /
    // / @note The classes IDDCore and IddictionSDK have been deprecated, please use Xplode as an entry point.
    ...
    public static native void initialize (String organizationSecret, UIApplicationLaunchOptions launchOptions,
        @Block InitializeHandler initializeHandler);

    // / Returns *YES* if initializeWithAppHandle:appSecret:onCompletion: has been called and successfully registered with the
// server.
    ...
    // / When a promotion has been set up using setupPromotionDockForBreakpoint:atPosition:, this method only presents the loaded
// promotion.
    // /
    ...
    public static native void presentPromotionForBreakpoint (String breakpoint,
        @Block PromotionBreakpointCompletionHandler completionHandler, @Block PromotionBreakpointDismissHandler dismissHandler);
    ...
    // /
    // / The state changes when the presentPromotionForBreakpoint:withCompletionHandler:andDismissalHandler:, dismissPromotion,
// removePromotionDock showPromotionDockAndBounce: and hidePromotionDock methods are called.
    // /
    ...
    // / Returns the visibility of a dockable promotion. The promotion is visible if the dock is visible or if it is presented
// full-screen.
    ...
    // / Initializes a dockable always-on-screen promotion for a specific breakpoint and optionally shows it at a specific
// position.
    // /
    // / @param breakpoint Name of the breakpoint.
    ...
    public static native void setupPromotionDock (String breakpoint, XplodeConstants.XPLPromotionDockingPosition position,
        @Block PromotionDockForBreakpointCompletionHandler completionHandler);

    // Initializes a dockable always-on-screen promotion for a specific breakpoint and dimensions, optionally shown at a specific
// position.
    // /
    ...
    public static native void setupPromotionDock (String breakpoint, XplodeConstants.XPLPromotionDockingPosition position,
        XplodeConstants.XPLPromotionDockDimensions dimensions,
        @Block PromotionDockForBreakpointCompletionHandler completionHandler);
    ...
    // /
    // / @note The promotion dock can only be shown after the promotion has been set up using
// setupPromotionDockForBreakpoint:atPosition:.
    ...
    // /
    // / @note The promotion dock can only be hidden after the promotion has been set up using
// setupPromotionDockForBreakpoint:atPosition: or shown using showPromotionDockAndBounce:.
    ...
    // /
    // / If you want to disable certain orientations for different view controllers, call this method before presenting any
// promotions.
    // /
    // / @note This method is optional.
    // / @param allowedOrientations Allowed promotion orientations. For possible values, see Apple's UIApplication documentation
// about UIInterfaceOrientationMask.
    ...
    // /
    // / @param deviceToken The unparsed device token that is returned by the UIApplication push registration delegate method as
// NSData.
    ...
    // /
    // / @param deviceToken The parsed device token string that is returned by the UIApplication push registration delegate
// method.
    // / @param completionHandler The completion handler.
    ...
    // /
    // / @param userInfo The received push payload that is returned by application:didReceiveRemoteNotification: as an
// NSDictionary.
    ...
    public static native void handlePushNotificationPayload (NSDictionary userInfo,
        @Block PushNotificationPayloadFetchCompletionHandler parameters);

    // ************** Xplode+Rewards.h *************

    // / To provide custom UI for sharing affiliate links, use this method. Before using it, configure breakpoint with rewarded
// content on the Xplode portal.
    // / @param breakpoint The name of the breakpoint configured with the rewarded content
    // / @param completionHandler A block that is executed when the shareLink is loaded from the server. Parameter shareLink may
// be nil if rewarded content is not configured or if an error occurs while downloading content
    // + (void)affiliateLinkForBreakpoint:(NSString*)breakpoint withCompletionHandler:(void
// (^)(NSURL*shareLink))completionHandler;
    @Method(selector = "affiliateLinkForBreakpoint:withCompletionHandler:")
    public static native void affiliateLinkForBreakpoint (String breakpoint,
        @Block AffiliateLinkCompletionHandler completionHandler);

    // / Call this method to load all awailable rewards for this user.
    // / @param completionHandler A block that is executed when the rewards are loaded from the server. Parameter rewards contains
// objects of type XPLReward. Parameter rewards may be nil if rewards are not available or if an error occurs while downloading
// content
