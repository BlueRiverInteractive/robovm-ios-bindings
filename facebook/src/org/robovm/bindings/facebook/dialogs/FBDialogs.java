...
public class FBDialogs extends NSObject {
     ...
     * dialog is only available on iOS 6.0 and above. The current active session returned by [FBSession activeSession] will be
     * used to determine whether the dialog will be displayed. If a session is active, it must be open and the login method used
     * to authenticate the user must be native iOS 6.0 authentication. If no session active, then whether the call succeeds or not
     ...
     * 
     * @param initialText The text which will initially be populated in the dialog. The user will have the opportunity to edit
     * this text before posting it. May be nil.
     * 
     ...
     * (and there is no active session), then whether the call succeeds or not will depend on whether Facebook integration has
     * been configured.
     * 
     * @param viewController The view controller which will present the dialog.
     * 
     * @param session The <FBSession> to use to determine whether or not the user has been authenticated with iOS native
     *            authentication. If nil, then [FBSession activeSession] will be checked. See discussion above for the
     *            implications of nil or non-nil session.
     * 
     * @param initialText The text which will initially be populated in the dialog. The user will have the opportunity to edit
     *            this text before posting it. May be nil.
     * 
    ...
    /** Determines whether a call to presentFBShareDialogWithTarget: will successfully present a dialog in the Facebook
    ...
    public static native boolean canPresentShareDialog (FBLinkShareParams params);
     ...
     * share is complete, the Facebook app will redirect to a url of the form "fb{APP_ID}://" that the application must handle.
     * The app should then call [FBAppCall handleOpenURL:sourceApplication:fallbackHandler:] to trigger the appropriate handling.
     * Note that FBAppCall will first try to call the completion handler associated with this method, but since during an app
     * switch, the calling app may be suspended or killed, the app must also give a fallbackHandler to the handleOpenURL: method
     * in FBAppCall.
     * 
     * @param params The parameters for the FB share dialog.
     * 
    ...
    public static native FBAppCall presentShareDialog (FBLinkShareParams params, NSDictionary<?, ?> clientState,
     ...
     * @return An FBAppCall object that will also be passed into the provided FBAppCallCompletionHandler.
     * 
     * @discussion A non-nil FBAppCall object is only returned if the corresponding canPresetFBShareDialogWithParams method is
     * also returning YES for the same params.
     */
     ...
     * @return YES if the dialog would be presented, and NO if not
     * 
     * @discussion A return value of YES here indicates that the corresponding presentFBShareDialogWithOpenGraphActionParams
     * method will return a non-nil FBAppCall for the same params. And vice versa.
     */
