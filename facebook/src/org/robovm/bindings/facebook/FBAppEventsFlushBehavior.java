...
public enum FBAppEventsFlushBehavior implements ValuedEnum {
	...
	/** Only flush when the `flush` method is called. When an app is moved to background/terminated, the events are persisted and
	 * re-established at activation, but they will only be written with an explicit call to `flush`. */
	ExplicitOnly;
