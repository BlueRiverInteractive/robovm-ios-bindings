...
public enum FacebookPermission {
    ...
    private FacebookPermission (String value, FBSessionAuthorizationType type) {
        this.name = value;
        this.type = type;
    }

    public static FacebookPermission[] getPermissionsByType (FBSessionAuthorizationType type) {
        List<FacebookPermission> list = new ArrayList<>();
        for (FacebookPermission permission : values()) {
            if (permission.getType() == type) {
                list.add(permission);
            }
        }
        return list.toArray(new FacebookPermission[list.size()]);
    }
    ...
    public static FacebookPermission fromName (String name) {
        for (FacebookPermission permission : values()) {
            if (permission.getName().equals(name)) return permission;
        }
        return UNKNOWN;
    }

    public static String[] getNamesOfPermissions (FacebookPermission[] permissions) {
        int n = permissions.length;
        String[] list = new String[n];
        for (int i = 0; i < n; i++) {
            list[i] = permissions[i].getName();
        }
        return list;
    }
}
