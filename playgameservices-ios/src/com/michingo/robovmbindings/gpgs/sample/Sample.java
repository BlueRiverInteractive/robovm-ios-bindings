...
public class Sample extends UIApplicationDelegate.Adapter implements GPPSignInDelegate, GPGAchievementControllerDelegate, GPGLeaderboardControllerDelegate, GPGLeaderboardsControllerDelegate {
	...
	private OnTouchUpInsideListener b1tl, b2tl, b3tl, b4tl, b5tl, b6tl, b7tl, b8tl, b9tl, b10tl, b11tl, b12tl, b13tl, b14tl, b15tl;
		...
		b5tl = new OnTouchUpInsideListener() {
			...
			public void onTouchUpInside(UIControl control, UIEvent event){
				if (signedIn){
					...
					/*NSArray<GPGAchievementMetadata> arr = GPGManager.sharedInstance().applicationModel().achievement().allMetadata();
					...
					incrementBlock = new GPGAchievementDidIncrementBlock(){
						...
						public void invoke(boolean newlyUnlocked, int currentSteps, NSError error) {
							if (error!=null){
								...
								System.out.println("increment succeeded. newlyUnlocked: "+newlyUnlocked+", currentSteps: "+currentSteps);
							}
						}
					};
		...
		b11tl = new OnTouchUpInsideListener() {
			...
			public void onTouchUpInside(UIControl control, UIEvent event){
				if (signedIn){
					...
					System.out.println("Your avatar url is located here: "+GPGManager.sharedInstance().applicationModel().player().localPlayer().avatarUrl());
					System.out.println("Your playerId is: "+GPGManager.sharedInstance().applicationModel().player().localPlayer().playerId());
				}else{
					System.out.println("sign in first.");
				}
			}
		...
		b15tl = new OnTouchUpInsideListener() {
			...
			public void onTouchUpInside(UIControl control, UIEvent event){
				if (signedIn){
					...
					//java.lang.ClassCastException: org.robovm.cocoatouch.foundation.NSObject cannot be cast to com.michingo.robovmbindings.gpgs.GPGLeaderboardMetadata
					
					/*//obtain all metadata for the leaderboards
					NSArray<GPGLeaderboardMetadata> allLeaderboards = .allMetadata();
					
					//iterate through the leaderboards
					for(GPGLeaderboardMetadata leaderboard : allLeaderboards){
	...
	public void loadState(){
		...
		cloudLoadCompletionHandler = new GPGAppStateLoadResultHandler() {
			...
			public void invoke(GPGAppStateLoadStatus status, NSError error) {
				switch(status){
					case GPGAppStateLoadStatusNotFound:System.out.println("cloud load failed: not found. This must be the first time the user plays the game.");break;
					case GPGAppStateLoadStatusUnknownError:System.out.println("cloud load failed: unknown error");break;
