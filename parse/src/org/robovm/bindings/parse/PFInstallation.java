 ...
 * currentInstallation] because the required identifier fields are readonly. The
 * timeZone and badge fields are also readonly properties which are
 * automatically updated to match the device's time zone and application badge
 * when the PFInstallation is saved, thus these fields might not reflect the
 * latest device state if the installation has not recently been saved.
 * 
 * PFInstallation objects which have a valid deviceToken and are saved to the
 * Parse cloud can be used to target push notifications.
 * 
...
// + (PFQuery *)query;
    ...
    /**
     * Gets the currently-running installation from disk and returns an instance
     * of it. If this installation is not stored on disk, returns a
     * PFInstallation with deviceType and installationId fields set to those of
     * the current installation.
     * 
     * @return a PFInstallation that represents the currently-running
     *         installation.
     */
    ...
    @Property
    public native void setBadge(@MachineSizedSInt long badge);
